...
public class DiscoveryDAOImpl implements DiscoveryDAO{
	...
	private final static String READ_DISCOVERY = "SELECT user.user_id,username,email,is_active,password,discovery_id,name,description,url,date,up_vote,down_vote "
			+"FROM discovery LEFT JOIN user ON discovery.user_id=user.user_id WHERE discovery_id=:discovery_id;";
	private final static String UPDATE_DISCOVERY ="UPDATE discovery SET name=:name,description=:description,url=:url,user_id=:user_id,date=:date,up_vote=:up_vote,down_vote=:down_vote "
	...
	public List<Discovery> getAll() {
		List<Discovery> discovery = template.query(READ_ALL_DISCOVERIES, new DiscoveryRowMapper());
		return discovery;
	}

	
	/*
	 * klasa wewnêtrzna DiscoveryRowMapper, która pozwala na rzutowanie wiersza danych z bazy danych na obiekt typu Discovery z uwzglêdnieniem utworzenia obiektu typu User.
	 */
